...
// Patchwork to make this work in Linux
#include <stdlib.h>     /* srand, rand */
#include <cstring>      /* strcpy */
#include <time.h>       /* time */
uint randomint(int maximum_inclusive) {
	return rand() % 10;
}
...
void getFullLine(char * buffer, bool constructive) {
  ...
  int verbNumber = randomint(constructive ? numberOfConstructiveVerbs : numberOfDestructiveVerbs);
  int verbLength = strlen(constructive ? constructiveVerbs[verbNumber] : destructiveVerbs[verbNumber]);
  int nounNumber = randomint(numberOfNouns);
  ...
  while ((constructive ? verbNumber == lastConstructiveVerbNumber : verbNumber == lastDestructiveVerbNumber)
    ...
    verbNumber = randomint(constructive ? numberOfConstructiveVerbs : numberOfDestructiveVerbs);
    verbLength = strlen(constructive ? constructiveVerbs[verbNumber] : destructiveVerbs[verbNumber]);
    nounNumber = randomint(numberOfNouns);
    nounLength = strlen(nouns[nounNumber]);
  }

  std::string output = std::string(constructive ? constructiveVerbs[verbNumber] : destructiveVerbs[verbNumber]);
  output += "ing ";
  
  output += nouns[nounNumber];
  if (output.length() < 18) output += "...";

  std::strcpy (buffer, output.c_str());
